...
export default function Navbar() {
  ...
  return (
    <motion.nav
      ...
      animate={{ y: 0, opacity: 1 }}
      transition={{ duration: 0.6 }}
      className="fixed top-0 left-0 w-full z-50 flex justify-between items-center px-6 md:px-12 py-2 bg-gradient-to-r from-indigo-900 to-purple-900 text-white shadow-lg"
    >
      {/* Logo */}
      ...
      {isOpen && (
        <motion.ul
          ...
          className="absolute top-full left-0 w-full bg-indigo-900 flex flex-col items-center gap-4 py-4 md:hidden"
        >
          {navItems.map((item) => (
            <li key={item.name}>
              <Link
